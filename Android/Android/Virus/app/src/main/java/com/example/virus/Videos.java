...
public class Videos extends Activity implements OnItemClickListener,JsonResponse {
	...
	String[] videoid, video_audio,value;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String q = "/public_view_videos?rid="+Viewrequest.rid;
		q = q.replace(" ", "%20");
		JR.execute(q);
	}
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		in.putExtra("vdo", video_audio[arg2]);
		startActivity(in);
	}
			...
			if (method.equalsIgnoreCase("public_view_videos")) {
				...
				if (status.equalsIgnoreCase("success")) {
					...
					for (int i = 0; i < ja1.length(); i++) {
						...
						value[i] =  "video_audio:" + video_audio[i]    ;
...
//				 lv1.setAdapter(clist);
					...
					ArrayAdapter<String> ar = new ArrayAdapter<String>(getApplicationContext(), android.R.layout.simple_list_item_1, video_audio);
					lv1.setAdapter(ar);
