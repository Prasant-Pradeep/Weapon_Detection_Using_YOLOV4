...
public class Userviewrequest extends AppCompatActivity implements JsonResponse, AdapterView.OnItemClickListener {
    ...
    public static String audio, id, rid, tlati, tlongi, types;
    ...
    public void response(JSONObject jo) {
        try {
            ...
            if(method.equalsIgnoreCase("UserViewrequestupdate")) {
            ...
            else if (method.equalsIgnoreCase("UserViewrequest")) {
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
        ...
        if (types.equalsIgnoreCase("Audio")) {
            ...
            final CharSequence[] items = {"Forward To Police","Audio","Locations", "Cancel"};
            AlertDialog.Builder builder = new AlertDialog.Builder(Userviewrequest.this);
            builder.setItems(items, new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int item) {

                    if (items[item].equals("Forward To Police")) {

                        JsonReq JR = new JsonReq();
                        JR.json_response = (JsonResponse) Userviewrequest.this;
                        String q = "/UserViewrequestupdate?log_id=" + sh.getString("log_id", "") + "&rid=" + rid;
                        q = q.replace(" ", "%20");
                        JR.execute(q);

                        Toast.makeText(getApplicationContext(), "Forward To Police", Toast.LENGTH_LONG).show();
                        ...
                        Toast.makeText(Userviewrequest.this, ""+url, Toast.LENGTH_SHORT).show();
            ...
            final CharSequence[] items = {"Image", "Forward To Police","Locations", "Cancel"};
            AlertDialog.Builder builder = new AlertDialog.Builder(Userviewrequest.this);
            builder.setItems(items, new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int item) {
                    if (items[item].equals("Image")) {
                        startActivity(new Intent(getApplicationContext(), userImageView.class));
                    }

                else if (items[item].equals("Forward To Police")) {
